...
替代（conda 自带的 Python 与系统 Python 并存容易冲突；笔者近期已将个人 Python 包管理迁移至 [uv](https:
//docs.astral.sh/uv/)，但 uv 托管 ROS 2 工作区、配合 `--system-site-packages`
的方案**尚未实测**，故本仓库仍以 `.venv` 为准）。因此，**书中第 4 / 7 / 8 章需要安装第三方 Python 库时，
本仓库统一使用 `.venv` 虚拟环境**：
...
> 里安装自己的 `colcon`（`pip install --ignore-installed colcon-common-extensions`），
...
完整笔记（前置安装、zsh 自动激活、venv vs conda 对比、4 个踩坑详解、一键启动脚本模板）见
**`Docs/About pyvenv.md`**。
